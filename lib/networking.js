...
Networking.prototype.start = function () {
  ...
  // make a shim for chrome.system.network.getNetworkInterfaces(callback) ?
  ...
  var port_any = 0 // was previously 5353
  if (this.INADDR_ANY) {
    this.createSocket(index++, 'pseudo multicast',
      '0.0.0.0', port_any, this.bindToAddress.bind(this));
  }
};
...
Networking.prototype.createSocket = function (
  ...
  var sock;
  if (process.versions.node && semver.gte(process.versions.node, '0.11.13')) {
...
Networking.prototype.send = function (packet) {
  ...
  function onEach(connection) {
    ...
    if (false && sock.address().address === '0.0.0.0' && !this.INADDR_ANY) {
